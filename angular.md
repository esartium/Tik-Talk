...
toObservable - конвертация сигнала в observable (типа стрим).

Работает только в injection-контексте, то есть либо в конструкторе, либо при инициализации какого-то поля класса.

##### .

```
grid-auto-flow: column; // элементы этого блока будут расположены в виде горизонтального ряда
```

## Директивы

Компоненты без шаблона; пишем в них какую-то логику, чтобы с помощью переиспользования этого компонента переиспользовать её.

Генерация:

```
ng g d папка/название-директивы
```
...
Здесь [(ngModel)]="avatar" - это двустороннее связывание (two-way binding);
То есть:
в [] к атрибуту байндится переменная; () - это событие;
А тут делаем [()] - это значит, что что бы ни изменилось в инпуте, оно будет записано в переменную, которую байндим (тут это avatar; инициализируется она в ts-файле этого компонента, а байндим её к элементу в шаблоне этого компонента);
При этом работает ещё и так, что если инпут изменится, то avatar изменится, и наоборот тоже.
ngModel плохо работает с файловыми инпутами, но с обычными типами данных все норм.

## Атрибут RouterLinkActive

Говорит, какой класс повесить на элемент, имеющий такой атрибут, если путь в урле совпадает с его RouterLink.